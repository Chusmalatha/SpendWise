...
const AuditForm = () => {
  ...
  const validateStep1 = () => {
    const errs = {};
    if (!companyInfo.companyName.trim()) errs.companyName = 'Company name is required';
    if (!companyInfo.email.trim()) errs.email = 'Email is required';
    else if (!/\S+@\S+\.\S+/.test(companyInfo.email)) errs.email = 'Enter a valid email';
    if (!companyInfo.teamSize) errs.teamSize = 'Team size is required';
    setErrors(errs);
  ...
  const validateStep2 = () => {
    const errs = {};
    const hasValidTool = tools.some((t) => t.toolName && t.monthlySpend);
    if (!hasValidTool) errs.tools = 'Add at least one tool with a name and monthly spend';
    setErrors(errs);
    return Object.keys(errs).length === 0;
  };
  ...
  return (
    <div className="min-h-screen bg-dark-900 pb-16 relative overflow-hidden" style={{ paddingTop: '100px' }}>
      {/* Background orbs */}
      ...
      <div className="container-custom relative z-10 max-w-3xl">
        ...
        >
          <span className="section-label mb-4">AI Spend Audit</span>
          <h1 className="font-display text-4xl md:text-5xl font-800 text-white mb-4 mt-4" style={{ fontWeight: 800 }}>
            Audit Your <span className="gradient-text">AI Stack</span>
          </h1>
          <p className="text-slate-400 text-lg max-w-lg mx-auto">
            Add your AI tools below. We'll analyze your spend and find savings in under 4 minutes.
          </p>
        </motion.div>
        ...
        >
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex items-center">
              ...
              {i < STEPS.length - 1 && (
                <div className={`w-8 h-px transition-colors ${step > s.id ? 'bg-green-500/30' : 'bg-white/10'}`} />
              )}
            </div>
          ))}
        </motion.div>
            ...
            >
              ...
              {totals.totalMonthly > 0 && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="glass border border-white/10 rounded-2xl p-4 mb-5 flex items-center justify-between"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-indigo-500/20 flex items-center justify-center">
                      <MdInfo className="text-indigo-400" />
                    </div>
                    <span className="text-slate-400 text-sm">Total tracked spend</span>
                  </div>
                  <div className="flex gap-6 text-right">
                    <div>
                      <div className="text-white font-bold">{formatCurrency(totals.totalMonthly)}/mo</div>
                      <div className="text-slate-500 text-xs">{formatCurrency(totals.totalAnnual)}/yr</div>
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.99 }}
                className="w-full py-4 rounded-2xl border-2 border-dashed border-white/15 text-slate-400 hover:text-white hover:border-indigo-500/40 hover:bg-indigo-500/5 transition-all duration-300 flex items-center justify-center gap-2 font-medium"
              >
                <MdAdd size={20} />
                Add Another AI Tool
              </motion.button>
