        ...
        >
          ...
          >
            <div className="max-w-3xl">
              ...
              <h1 className="font-display text-4xl md:text-5xl xl:text-6xl text-white leading-tight font-extrabold">
                AI Spend <span className="gradient-text">Intelligence</span>
              </h1>
            ...
            {stats.map((stat, i) => (
              ...
              >
                <div className="flex items-start justify-between mb-5">
                  <div className={`w-11 h-11 rounded-2xl bg-${stat.color}-500/10 flex items-center justify-center text-xl`}>
                    {stat.icon}
                  </div>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between px-1 min-h-[48px] mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-white font-bold text-xl tracking-tight">Optimization Roadmap</h2>
                </div>
              ...
              {data.recommendations.length > 0 ? (
                <ResultCard key={data.recommendations[0].id} rec={data.recommendations[0]} index={0} />
              ) : (
                <motion.div
            ...
            <div className="space-y-5 self-start min-w-0 w-full xl:w-[390px] shrink-0 sticky top-24">
              <div className="flex items-center justify-between px-1 min-h-[48px]">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-white font-bold text-xl tracking-tight">Financial Delta</h2>
                </div>
              </div>
              ...
              >
                ...
                <div className="h-[290px] w-full mt-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.spendBreakdown} barGap={8} margin={{ top: 0, right: 0, left: -25, bottom: 0 }}>
                      ...
                      <XAxis dataKey="name" tick={{ fill: '#94a3b8', fontSize: 10, fontWeight: 500 }} axisLine={false} tickLine={false} dy={10} />
                      <YAxis tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} />
                      <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,0.03)' }} />
                      <Bar dataKey="spend" name="Current" radius={[8, 8, 0, 0]} barSize={16}>
                        {data.spendBreakdown.map((_: any, i: number) => <Cell key={i} fill="url(#colorCurrent)" />)}
                      </Bar>
                      <Bar dataKey="recommended" name="Recommended" radius={[8, 8, 0, 0]} barSize={16}>
                        {data.spendBreakdown.map((_: any, i: number) => <Cell key={i} fill="url(#colorRecommended)" />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                <div className="flex items-center justify-center gap-6 mt-6 pt-5 border-t border-white/5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Current</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.4)]" />
                    <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Target</span>
                  </div>
                </div>
              </motion.div>
            </div>
          ...
          {data.recommendations.slice(1).length > 0 && (
            <div className="mt-12 space-y-4">
              <h3 className="text-white/50 text-xs font-bold uppercase tracking-[0.2em] px-1 mb-6">Additional Optimization Steps</h3>
              <div className="space-y-4">
                {data.recommendations.slice(1).map((rec: any, i: number) => (
                  <ResultCard key={rec.id} rec={rec} index={i + 1} />
                ))}
              </div>
            </div>
          )}
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="mt-16 pt-10 pb-20 border-t border-white/5 text-center">
            <div className="flex flex-col sm:flex-row gap-10 justify-center">
              ...
              <Link to="/audit" className="px-8 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-bold flex items-center justify-center gap-2 hover:bg-white/10 transition-all">
                Start New Audit <MdArrowForward />
              </Link>
            </div>
          </motion.div>
        </div>
